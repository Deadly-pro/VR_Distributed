...
#include <string>
#include <vector>
#include <iostream>// Keep mapped memory persistent to avoid remapping every frame
    ...
    try {
        ...
        for (const auto& hand : parsed) {
            ...
            for (const auto& lm : hand["landmarks"]) {
                Vector3 pt = {
                    static_cast<float>((lm["x"].get<double>() - 0.5) * 0.3),
                    static_cast<float>((0.5 - lm["y"].get<double>()) * 0.3),
                    static_cast<float>(-lm["z"].get<double>() * 0.3)
                };
                points.push_back(pt);
            }
